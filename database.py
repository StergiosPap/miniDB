...
import shutil
from misc import split_condition, convert_to_type
...
class Database:
    ...
    def execSQL(self, query):
        '''
        Executes the corresponding miniDB function depending on the SQL command (using the parameters from sql_helper.py)
        '''
        ...
        if (re.match("(?i)^[\s]*SELECT .+ FROM .+", query)): ### SELECT ###            
            columns, table, innerjoin_table, innerjoin_condition, condition, order_by, asc_order, limit = parameters_select(query)
            ...
            if (re.match("(?i)^[\s]*SELECT .+ FROM .+ INNER JOIN .+ ON .+", query)): #with INNER JOIN
                ...
                self.inner_join(table, innerjoin_table, innerjoin_condition, return_object=True)._select_where('*', condition, order_by=order_by, asc=asc_order, top_k=limit).show()
            else: #without INNER JOIN
                self.select(table, columns, condition, order_by=order_by, asc=asc_order, top_k=limit)

        elif (re.match("(?i)^[\s]*INSERT INTO .+ SELECT .+", query)): ### INSERT + SELECT ###            
            save_as, select_query, columns, table, innerjoin_table, innerjoin_condition, condition, order_by, asc_order, limit = parameters_insert_select(query)
            
            if (re.match("(?i)^[\s]*SELECT .+ FROM .+ INNER JOIN .+ ON .+", select_query)): #with INNER JOIN
                if (columns != '*'):
                    print ("You cannot select specific columns with 'Inner Join' yet. Showing all columns...\n")
                
                self.inner_join(table, innerjoin_table, innerjoin_condition, return_object=True)._select_where('*', condition, order_by=order_by, asc=asc_order, top_k=limit, save_as=save_as).show()
            else: #without INNER JOIN
                self.select(table, columns, condition, order_by=order_by, asc=asc_order, top_k=limit, save_as=save_as)            

        elif (re.match("(?i)^[\s]*INSERT INTO .+ VALUES .+", query)): ### INSERT ###        
            table, values = parameters_insert(query)
            
            self.insert(table, values)        
        
        elif (re.match("(?i)^[\s]*UPDATE .+ SET .+ WHERE .+", query)): ### UPDATE ###            
            table, changes, condition, column, value = parameters_update(query)
            
            self.update(table, value, column, condition)            

        elif (re.match("(?i)^[\s]*DELETE FROM .+ WHERE .+", query)): ### DELETE ###                    
            table, condition = parameters_delete(query)
            
            self.delete(table, condition)        
        
        elif (re.match("(?i)^[\s]*CREATE TABLE .+", query)): ### CREATE TABLE ###            
            name, colnames, coltypes = parameters_create_table(query)
            
            self.create_table(name, colnames, coltypes)
            
        elif (re.match("(?i)^[\s]*DROP TABLE .+", query)): ### DROP TABLE ###        
            table = parameters_drop_table(query)
            
            self.drop_table(table)
            
        elif (re.match("(?i)^[\s]*CREATE INDEX .+ ON .+", query)): ### CREATE INDEX ON ###       
            name, table = parameters_create_index(query)
            
            self.create_index(table, name, 'Btree')            
            
        elif (re.match("(?i)^[\s]*DROP INDEX .+", query)): ### DROP INDEX ###       
            name = parameters_drop_index(query)
